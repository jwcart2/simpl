...
local function get_mls_range(state)
   local range = {}
   if lex_peek(state.lex) ~= "{" then
	  range[#range+1] = get_identifier(state, "sensitivity")
   return range
end
...
local function get_context(state)
   ...
   if lex_peek(state.lex) == "}" then
	  ...
	  context[#context+1] = get_range(state)
	  get_expected(state, "}")
   end
   return context
end
...
local blocks = {
   ["ifdef"] = parse_conditional_block,
   ["boolif"] = parse_conditional_block,
   ["tunif"] = parse_conditional_block,
...
-------------------------------------------------------------------------------	 
local function parse_simpl_policy(in_files, inactive_files, verbose)
   MSG.verbose_out("\nParse SIMPL Policy", verbose, 0)
...
simpl_parse.parse_simpl_policy = parse_simpl_policy
